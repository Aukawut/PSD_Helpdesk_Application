...
import { ProcessAll } from "./ProcessComponents";
interface PropsModal {
  handleClose: () => void;
  callBackRender: () => void;
  open: boolean;
  oldValue: ProcessAll | undefined;
}

const EditProcessModal: React.FC<PropsModal> = ({
  handleClose,
  open,
  callBackRender,
  oldValue,
  ...
  const handleUpdateSubmit = async (e: React.FormEvent) => {
    ...
    await axios
      .put(
        `${baseURL}/process/update/${oldValue?.id_process}`,
        {
          name_process: name,
          desc: desc,
  ...
  const setInit = () => {
    if (oldValue !== undefined) {
      setName(oldValue?.name_process);
      setDesc(oldValue?.desc_process);
    }
  };
  useEffect(() => {
    if (open) {
        ...
        <Box sx={style}>
          ...
          >
            <div>
              ...
              >
                Update Process Master # {oldValue?.id_process}
              </Typography>
            </div>
